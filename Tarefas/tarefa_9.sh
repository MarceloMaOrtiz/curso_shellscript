...
#       - Adicionando cores
#   v1.3 02/07/2024, Autor da Mudança: Marcelo
#       - Adicionando arquivo de configs.cf para Nomes,
#       types e status
#
# ------------------------------------------------------- #
# Testado em:
...
PATH_CONFIGURACAO="$PATH_SH/Files/pokedex_config.cf"
PATH_PARSER="Files/parser.sh"
...
eval $(./$PATH_PARSER $PATH_CONFIGURACAO)

[ "$(echo $CONF_CHAVE_NOMES)" = 1 ] && \
    CHAVE_NOMES=1

[ "$(echo $CONF_CHAVE_TYPES)" = 1 ] && \
    CHAVE_TYPES=1

[ "$(echo $CONF_CHAVE_STATUS)" = 1 ] && \
    CHAVE_STATUS=1
